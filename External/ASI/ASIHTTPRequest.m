...
- (void)main
{
	@try {
		...
		[[self cancelledLock] lock];//递归锁，多次调用不会阻塞已获取该锁的线程
		
		#if TARGET_OS_IPHONE && __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_4_0
		if ([ASIHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {//支持多任务并且允许再后台继续执行
            ...
            //beginBackgroundTaskWithExpirationHandler和endBackgroundTask保证在程序突然切入后台的时候保证任务不被中断
		}
		#endif


		// A HEAD request generated by an ASINetworkQueue may have set the error already. If so, we should not proceed.
        //队列里可能已经发生了一个错误，这种情况下，我们不能继续执行下去
		if ([self error]) {
			[self setComplete:YES];
			[self markAsFinished];
			return;		
