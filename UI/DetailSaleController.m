...
@interface DetailSaleController () {
    UIView *_garyView;
}

@end

@implementation DetailSaleController

- (void)dealloc {
    self.couponTwoArray = nil;
    [_garyView release];
    [super dealloc];
}
...
- (void)viewDidLoad
{
    ...
    [HTTPRequestEnding getCouponTwoComplete:^(NSMutableArray *couponTwoArray) {
        self.couponTwoArray = couponTwoArray;
        
        CouponTwo *couponTwo = [self.couponTwoArray objectAtIndex:_indexPathRow];
        
        UIWebView *web = [[UIWebView alloc] initWithFrame:CGRectMake(0, 0, 320, self.view.bounds.size.height)];
        NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://buy.mplife.com/home/ticket/wap-show/tid/%@",couponTwo.ticket_id]];
