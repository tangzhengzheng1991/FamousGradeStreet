...
@interface DetailEndController () {
    UIView *_garyView;
}

@end

@implementation DetailEndController
...
- (void)viewDidLoad
{
    ...
    [HTTPRequestEnding getCouponEndComplete:^(NSMutableArray *couponEndArray) {
        ...
        EndCoupon *endCoupon = [self.couponEndArray objectAtIndex:_indexPathRow];
        
        UIWebView *web = [[UIWebView alloc] initWithFrame:CGRectMake(0, 0, 320, self.view.bounds.size.height)];
        NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://buy.mplife.com/home/ticket/wap-show/tid/%@",endCoupon.ticket_id]];
